...
export default function Footer() {
    const gitAccounts = [
        ...
        {login: 'shevadead', link: 'https://github.com/sheVadead'},
        {login: 'zhenjahorbach', link: 'https://github.com/ZhenjaHorbach'}
    ];

    const allGitAccounts = gitAccounts.map(({login, link}) => {
        return (
            <li className="authors__item" key={login}>
                <a href={link} className="authors__link">
                    <span className="authors__name">{login}</span>
    ...
    return (
        <footer className="footer">
            <div className="details">
                <a href="https://rs.school/" className="rs-logo">
                    <img src={rsLogo} alt="rs-school" className="rs-logo__img"/>
                </a>
                <p className="copyright">© 2021 TravelApp</p>
            </div>
